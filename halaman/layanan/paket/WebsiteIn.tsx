...
import { LayoutTemplate, Check, Zap } from 'lucide-react';
...
export const WebsiteIn: React.FC<PackageProps> = ({ onNavigate, onDetail, data }) => {
  if (!data) return null;

  return (
     <div 
         onClick={onDetail}
         className="bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 hover:shadow-2xl hover:shadow-blue-900/10 transition-all duration-300 flex flex-col h-full group relative overflow-hidden ring-1 ring-slate-200 cursor-pointer"
     >
         {/* Banner Image */}
         <div className="w-full aspect-[4/3] relative overflow-hidden">
             <img loading="lazy" 
                 src="/gambar/pilihan-layanan2.webp" 
                 alt={data.nama}
         ...
         <div className="p-8 relative z-10 flex flex-col flex-grow">
            ...
            <ul className="space-y-3 mb-8 flex-grow">
                {data.manfaat && data.manfaat.slice(0, 5).map((feat: string, f: number) => (
            ...
            <div className="mb-6 flex items-center gap-2 border-t border-slate-100 pt-4">
                 <span className="text-[10px] text-slate-400 font-bold uppercase">Built With:</span>
